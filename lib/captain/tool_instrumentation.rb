module Captain::ToolInstrumentation
  ...
  def instrument_tool_session(params)
    return yield unless IntelychatApp.otel_enabled?
    ...
    IntelychatExceptionTracker.new(e, account: account).capture_exception
  ...
  def record_generation(chat, message, model)
    return unless IntelychatApp.otel_enabled?
