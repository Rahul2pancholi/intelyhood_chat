module Integrations::Slack::SlackMessageHelper
  def process_message_payload
    ...
    IntelychatExceptionTracker.new(e, account: conversation.account).capture_exception
  ...
  def resolve_slack_sender
    ...
    intelychat_user = conversation.account.users.from_email(slack_user[:profile][:email])
    return [intelychat_user, nil, nil] if intelychat_user
