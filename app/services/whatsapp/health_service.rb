class Whatsapp::HealthService
  ...
  def initialize(channel)
    ...
    # Health lookups need fields only available on newer Graph API versions, independent of
    # whatever version the account otherwise sends/receives messages on.
  ...
  # Fetches fresh health data from Meta and persists it, so the last known-good snapshot
  # survives even if a later refresh fails or Meta is temporarily unreachable.
