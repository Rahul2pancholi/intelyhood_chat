class Notification::PushTestService
  ...
  def self.default_title
    format(DEFAULT_TITLE, installation_name: GlobalConfigService.load('INSTALLATION_NAME', 'Intelychat'))
  end
  ...
  def test_fcm_via_hub(subscription)
    response = IntelychatHub.send_push_with_response(fcm_options(subscription))
  ...
  def browser_push_payload(subscription)
    {
      message: JSON.generate(
        ...
        url: ENV.fetch('FRONTEND_URL', 'https://app.intelychat.com')
      ),
      endpoint: subscription.subscription_attributes['endpoint'],
      p256dh: subscription.subscription_attributes['p256dh'],
      auth: subscription.subscription_attributes['auth'],
      vapid: {
        subject: ENV.fetch('FRONTEND_URL', 'https://app.intelychat.com'),
