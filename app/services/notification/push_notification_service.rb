class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_intelychat_hub(subscription)
    end
  end
  ...
  def handle_browser_push_error(error, subscription)
    ...
    when Errno::ECONNRESET, Net::OpenTimeout, Net::ReadTimeout, Socket::ResolutionError
      ...
      IntelychatExceptionTracker.new(error, account: notification.account).capture_exception
      true
    end
  end
  ...
  def send_push_via_intelychat_hub(subscription)
    return if firebase_credentials_present?
    return unless intelychat_hub_enabled?
    return unless subscription.fcm?

    IntelychatHub.send_push(fcm_options(subscription))
  end
