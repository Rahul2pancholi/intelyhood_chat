class UserSessionTrackingService
  ...
  LEGACY_MOBILE_UAS = [
    ...
    { match: %r{\AIntelychat/.*CFNetwork.*Darwin}, platform: 'iPhone', device: 'iPhone' }
  ].freeze
  ...
  def mobile_client_headers
    name = @request.headers['X-Intelychat-Client-Name']
    return nil if name.blank?

    platform = @request.headers['X-Intelychat-Platform']
    model = @request.headers['X-Intelychat-Device-Model']

    {
      browser_name: name,
      browser_version: @request.headers['X-Intelychat-Client-Version'],
      device_name: device_name_for_icon(platform, model),
      platform_name: model,
      platform_version: @request.headers['X-Intelychat-Platform-Version']
    }
  end
