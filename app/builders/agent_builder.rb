...
class AgentBuilder
  ...
  # Raises if adding another agent would exceed the account's plan limit.
  # Runs inside `account.with_lock` so concurrent agent-create requests can't
  # all observe the same remaining seat before any account_user row exists.
  def ensure_within_agent_limit!
    subscription = account.subscription
    return if subscription.nil? || subscription.within_agent_limit?

    raise CustomExceptions::Account::PlanUpgradeRequired, {}
  end
