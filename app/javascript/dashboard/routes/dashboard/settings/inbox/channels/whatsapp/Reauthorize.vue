...
const whatsappAppId = computed(() => window.intelychatConfig.whatsappAppId);
const whatsappConfigurationId = computed(
  () => window.intelychatConfig.whatsappConfigurationId
);
...
onMounted(async () => {
  try {
    ...
    await setupFacebookSdk(
      whatsappAppId.value,
      window.intelychatConfig?.whatsappApiVersion
    );
  } catch (error) {
