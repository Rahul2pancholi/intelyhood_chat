...
const fetchSSLStatus = () => {
  if (!isOnIntelychatCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on Intelychat cloud)
  if (portalObj?.custom_domain && isOnIntelychatCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
