...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.intelychatWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.intelychatWebChannel;
    ...
    registerCampaignEvents() {
      ...
      emitter.on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.intelychatWebChannel;
